...
if(!answer.todo){
 ...
 console.log(
    chalk.green(
        `"${chalk.red(answer.todo)}"your task has been successfully added to the list.\n`
  )
 );
...
if (condition = ques.choice){
    ...
    if (addmore.select == "Delete task"){
        ...
        let newtodo: any = todos.filter(val => val !== delettodo.delete)
        todos = [...newtodo]
    ...
    if (addmore.select == "Update list"){
        ...
        let newtodo: any = todos.filter(val => val !== updatetodo.delete)
